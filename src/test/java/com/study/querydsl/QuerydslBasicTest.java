...
//        List<Member> content = fetchResults.getResults();
    ...
    public void paging2() {
        ...
        assertThat(queryResults.getTotal()).isEqualTo(5);
    ...
    public void aggregation() {
        ...
        Tuple tuple = result.get(0); // 여러개 단위가 사용될 때, but 실무에선 DTO로 전환하는 방법 자주사용함
        assertThat(tuple.get(member.count())).isEqualTo(5);
        assertThat(tuple.get(member.age.sum())).isEqualTo(200);
    ...
    public void theta_join() {
        ...
        List<Member> result = queryFactory
                .select(member)
                .from(member, team) // theta에서는 연관관계 상관없이 from에서 나열을 바로해버린다
...
//                .join(member.team, team).on(team.name.eq("teamA"))
    ...
    /**
     * 순수 JPA에서 DTO 조회하려면 new 명령어로 package 명을 다 적어줘서
     * 지저분하고 생성자 방식만 지원하고 있다
     */
    @Test
    ...
    // 따로 빼놓고 활용하며 실무에서 재사용 빈도와 가독성이 높아진다!
